...
public class CreditCardBill {
	...
	public CreditCardBill (String
			creditCardNo, String customerId, Date
			BillDate, Transaction
			monthTransactions[])
	{
	...
	public double getTotalAmount(String transactionType)
	{
		...
		if(!(transactionType.equals("CR") || transactionType.contentEquals("DB")))
		{
			return 0.0;
		}
		else
		...
		try
		{
			if (validateData().equals("valid"))
			{
				if(monthTransactions!=null && monthTransactions.length>0)
				{
					...
					double intrest = (outStanding * 0.199)/12;
